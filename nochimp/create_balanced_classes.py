...
valid_split_percent = 3
multiclass_convnet_only = True
...
if not (os.path.exists(train_path % 0) and os.path.exists(valid_path % 0)):
    ...
    save_as_train_and_test(X_data, train_path % 0, valid_path % 0, valid_split_percent)

################################################################################
# Now do the other splitting parts

if not multiclass_convnet_only:
    for mi in [1,2,3,4]:
